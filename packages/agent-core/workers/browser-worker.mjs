...
function fail(request, code, message) {
  result(request, { ok: false, code, message });
}
function cleanValue(value) {
  const text = JSON.stringify(value ?? null);
  return text.length > MAX_OUTPUT
    ? { value: `${text.slice(0, MAX_OUTPUT)}...`, truncated: true }
...
async function handle(request) {
  ...
  if (request.method === "open") {
    ...
    await session.page.goto(url.href, {
      waitUntil: "domcontentloaded",
      timeout: timeout(params.timeoutMs),
    });
    result(request, {
      ok: true,
      sessionId,
  ...
  if (request.method === "screenshot") {
    ...
    await session.page.locator(String(params.selector)).click({
    ...
    const value = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error("页面脚本执行超时")), timeout(params.timeoutMs)),
      ),
    ]);
