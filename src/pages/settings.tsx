...
function CommitApiKeySection() {
  ...
  useEffect(() => {
    let isActive = true;
    void loadCommitApiKey().then((savedKey) => {
      if (isActive) setHasSavedKey(savedKey.trim().length > 0);
    });
    return () => {
      isActive = false;
    };
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    if (!apiKey) {
      setNotice("请输入 COMMIT_API_KEY。");
      return;
    }
    setIsSaving(true);
    setNotice("");
    try {
      await saveCommitApiKey(apiKey);
      setHasSavedKey(true);
      setDraftKey("");
...
function LookerApiKeySection() {
  ...
  useEffect(() => {
    let isActive = true;
    void loadLookerApiKey().then((savedKey) => {
      if (isActive) setHasSavedKey(savedKey.trim().length > 0);
    });
    return () => {
      isActive = false;
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    const apiKey = draftKey.trim();
    if (!apiKey) {
      setNotice("请输入 LOOKER_API_KEY。");
      return;
    }
    setIsSaving(true);
    setNotice("");
    try {
      await saveLookerApiKey(apiKey);
      setHasSavedKey(true);
      setDraftKey("");
      setNotice("已保存。密钥仅本地存储，不再回显。");
    } catch {
  ...
  return (
    <section className="rounded-lg border border-border bg-muted/60 p-5">
      ...
      <p className="mt-2 max-w-2xl text-muted-foreground text-sm leading-6">
        配置 Looker API Key 后，Looker 页面才能读取你的监控数据。 密钥仅保存在本机，保存后不再显示。
      </p>
      <form className="mt-4 flex flex-col gap-3" onSubmit={handleSave}>
        <div>
          <label
            className="block font-medium text-muted-foreground text-sm"
            htmlFor="looker-api-key"
          >
            LOOKER_API_KEY
          </label>
          <div className="mt-1.5 flex flex-wrap items-center gap-2">
