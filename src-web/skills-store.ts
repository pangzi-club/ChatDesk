...
function parseSkill(file: string, source: string): Promise<ServerSkill | null> {
  return Promise.all([stat(file), readFile(file, "utf8")]).then(([metadata, content]) => {
    ...
    for (const line of content.split(/\r?\n/).slice(0, 80)) {
      ...
      if (trimmed === "---") {
        ...
        name = trimmed.slice(5).trim().replace(/^['"]|['"]$/g, "") || name;
    ...
    if (!description) description = content.split(/\r?\n/).find((line) => line.trim() && !line.trim().startsWith("#"))?.trim().slice(0, 180) ?? "";
    return { id: `${source}:${directory.toLowerCase()}`, name, description, source, path: file, content };
  }).catch(() => null);
}

async function skillFiles(root: string) {
  const entries = await readdir(root, { withFileTypes: true }).catch(() => []);
  return entries.filter((entry) => entry.isDirectory()).map((entry) => path.join(root, entry.name, "SKILL.md"));
}
