...
export class McpRuntime {
  ...
  async start(value: unknown) {
    ...
    if (server.transport === "remote") {
      ...
      const client = await createMCPClient({ transport: { type: "http", url: server.url, headers: server.headers } });
  ...
  async callTool(serverId: string, toolName: string, arguments_: unknown) {
    const remote = this.remotes.get(serverId);
    if (remote) return remote.callTool({ name: toolName, arguments: arguments_ as Record<string, unknown> });
