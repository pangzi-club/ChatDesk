...
type BrowserResponse = { ok: boolean; sessionId?: string; data?: unknown; code?: string; message?: string };
...
export class BrowserRuntime {
  ...
  close() {
    ...
    for (const resolve of this.pending.values()) resolve({ ok: false, code: "closed", message: "浏览器 worker 已关闭" });
    this.pending.clear();
  }
  ...
  private ensureWorker() {
    ...
    const worker = spawn(command, args, {
      env: { ...process.env, ...(process.env.CHAT_SERVER_PLAYWRIGHT_BROWSERS_PATH ? { PLAYWRIGHT_BROWSERS_PATH: process.env.CHAT_SERVER_PLAYWRIGHT_BROWSERS_PATH } : {}) },
      stdio: "pipe",
    });
