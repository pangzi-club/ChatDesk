...
- **浏览器**：`client-tools.ts` + `browser-runtime.ts`，隔离的 headless Chromium session，open / screenshot / click / eval / close 全套。开发态回退到源码 `browser-worker.mjs`；打包后由 Tauri 注入 `CHAT_SERVER_BROWSER_WORKER`，缺失则 Chat Server 启动失败。
