...
const contributionLabels: Record<string, { label: string; description: string }> = {
  ...
  "chat.messages.empty": { label: "空屏定制", description: "替换聊天空屏的问候与建议卡片" },
  "chat.composer.float": { label: "输入区浮层", description: "在聊天输入框上方提供浮层内容" },
  "chat.message.action": { label: "消息操作", description: "为每条聊天消息提供操作按钮" },
  "chat.message.before": { label: "消息前置", description: "在每条聊天消息前方插入内容" },
  "chat.message.after": { label: "消息后置", description: "在每条聊天消息后方插入内容" },
  "chat.message.meta": { label: "消息元信息", description: "替换消息的状态与耗时展示" },
  "chat.generating": { label: "生成状态", description: "替换生成进行中的等待指示" },
  "chat.status": { label: "底部状态", description: "在聊天页底部提供状态内容" },
  "chat.theme": { label: "聊天主题", description: "覆盖聊天字体、字号与气泡样式变量" },
