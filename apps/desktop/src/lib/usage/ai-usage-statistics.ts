...
import { loadChatIndex, loadChatSession } from "@/lib/chat/chat-store";
import { loadChatServerAiUsageLogs, loadChatServerReviewerLogs } from "@/lib/server/chat-server";
import { loadModels, type ModelConfig } from "@/lib/server/models";
import {
  addTokenUsage,
  emptyTokenUsage,
  getMessageUsage,
...
export type UsageRecord = {
  date: string;
  source: ArchiveSource | "reviewer" | "chat-server";
  provider: string;
  model: string;
  usage: TokenUsage;
  messageCount: number;
...
export type UsageAggregate = {
  provider: string;
  model: string;
  source: ArchiveSource | "reviewer" | "chat-server" | "mixed";
  messageCount: number;
  usage: TokenUsage;
};
...
async function collectRecords(models: ModelConfig[]): Promise<UsageRecord[]> {
  ...
  let aiUsageLogs: Awaited<ReturnType<typeof loadChatServerAiUsageLogs>> = [];
  try {
    aiUsageLogs = await loadChatServerAiUsageLogs();
  } catch {
  ...
  for (const entry of aiUsageLogs) {
    ...
    const config =
      (entry.modelId ? modelById.get(entry.modelId) : undefined) ??
      (entry.model ? modelByName.get(entry.model) : undefined);
    records.push({
      date: dateKey(entry.timestamp, new Date()),
      source: "chat-server",
      provider: entry.provider ?? config?.provider ?? "未知供应商",
      model: entry.model ?? config?.name ?? entry.modelId ?? "Chat Server",
      usage: entry.usage,
      messageCount: 1,
    });
  }
