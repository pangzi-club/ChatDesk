...
import { joinTaskCwd } from "@/lib/workspace/workspace-path";
import { loadWorkspaceProjects } from "@/lib/workspace/workspaces";
...
function normalizeChatSession(value: unknown): ChatSession | null {
  ...
  return {
    ...
    source:
      value.source === "cli" || value.source === "feishu" || value.source === "automation"
        ? value.source
        : undefined,
