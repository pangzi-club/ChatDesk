...
export function filterSidebarConversations<T extends Pick<ChatIndexItem, "source">>(
  sessions: T[],
  showAllTasks = false,
) {
  if (showAllTasks) return sessions;
  return sessions.filter(
    (session) => session.source !== "feishu" && session.source !== "automation",
  );
}
